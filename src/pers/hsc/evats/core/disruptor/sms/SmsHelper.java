...
import pers.hsc.evats.core.utils.sms.sender.UcpaasSender;
...
public class SmsHelper {
	...
	public SmsResult sendSyncSmsByHuyi(String phone, SmsTemplate smsTemplate, String... datas) {
		// 异步的时候发送短信日志
		...
		SmsResult smsResult = smsSender.sendByHuyi(phone, smsTemplate, datas);
		if (smsDao != null) {
	...
	public SmsResult sendSyncSmsByUcpaas(String phone, SmsTemplate smsTemplate, String... datas) {
		// 异步的时候发送短信日志
		String eventId = StringUtils.randomUUID();
		SmsData smsData = new SmsData();
		smsData.setPhone(phone);
		...
		UcpaasSender ucpaasSender = new UcpaasSender();
		SmsResult smsResult = ucpaasSender.sendByUcpaas(phone, smsTemplate, datas);
		if (smsDao != null) {
			smsDao.doResult(eventId, smsData, smsResult);
		}
       return smsResult;
