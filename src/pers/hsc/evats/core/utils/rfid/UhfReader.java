...
//		String webClassesPath = UhfReader.class.getClass().getResource("/").getPath();
		String dllPath;
		String[] paths = new String[] { "/UhfReader_API.dll", "/../UhfReader_API.dll",
				"/../src/UhfReader/UhfReader_API.dll", "/../src/UhfReader_API.dll", "/../resources/UhfReader_API.dll" };
		for (int i = 0; i < paths.length; i++) {
			dllPath = parentFolder + paths[i];
//			dllPath = "D:/UhfReader/UhfReader_API.dll";
//			System.out.println(dllPath);
	...
	public boolean RecvData(StringBuffer PC, StringBuffer EPC, StringBuffer RSSI/* 10进制的字符串 */) {
		...
		if (inventorySingleLen > 0) {
			...
			// String CRC =
			// UhfReader.getHexStringFrombyte(inventorySingleBuf,2,inventorySingleLen-3);
			return true;
		}
		return false;
	}
	...
	private static String GetModuleFolder() {
		...
		String LOCATION = "";
		try {
			LOCATION = URLDecoder.decode(UhfReader.class.getProtectionDomain().getCodeSource().getLocation().getFile(),
					"UTF-8");
	...
	public int ReadDataByEPC(String accessPwd/* 000000,4字节默认密码，送入字符串 */, int memBank/* 详见文件开头UHF_DATA_SECTION_XX的定义 */,
			int sa, int dl, byte[] uDataReturn/* 返回的数据 */) {
	...
	public int ReadDataByTID(int sa, int dl, byte[] uDataReturn/* 硬件返回的原始数据 */,
	...
	public int ReadDataByXZEPC(String epcLabelStr/* 需要自动选中的标签 */,
			String accessPwd/* 000000,4字节默认密码，送入字符串 */, int memBank/* 详见文件开头UHF_DATA_SECTION_XX的定义 */, int sa, int dl,
			byte[] uDataReturn/* 返回的数据 */) {
	...
	public int WriteDataByEPCEx(String uAccessPwd/* 000000,4字节默认密码，送入字符串 */,
			...
			String uPtr/* 写内存的起始地址00 00 */, int uCnt/* 写的个数00 03 */, byte[] uWriteData/* 写入的数据 */,
			ByteByReference uErrorCode/* 返回的错误代码 */) {
		return ns.UhfWriteDataByEPCEx(handle, uAccessPwd, (byte) uBank, uPtr, (byte) uCnt, uWriteData, uErrorCode,
				(byte) flagCrc);
	}

	// 这个函数与UhfWriteDataByEPC函数的区别是写数据前，内部会自动做选择用户指定标签操作
	public int WriteDataByXZEPC(String EPCXZ,
			String uAccessPwd/* 000000,4字节默认密码，送入字符串 */, int uBank/* 详见文件开头UHF_DATA_SECTION_XX的定义 */,
			String uPtr/* 写内存的起始地址00 00 */, int uCnt/* 写的个数00 03 */, byte[] uWriteData/* 写入的数据 */,
			ByteByReference uErrorCode/* 返回的错误代码 */) {
		return ns.UhfWriteDataByXZEPC(handle, EPCXZ, uAccessPwd, (byte) uBank, uPtr, (byte) uCnt, uWriteData,
				uErrorCode, (byte) flagCrc);
	}

	// 这个函数与UhfWriteDataByEPC函数的区别是写数据前，内部会自动做选择标签操作，同时固定写入USER区
	public int WriteDataByUSER(String uAccessPwd/* 000000,4字节默认密码，送入字符串 */,
			String uPtr/* 写内存的起始地址00 00 */, int uCnt/* 写的个数00 03 */, byte[] uWriteData/* 写入的数据 */,
			ByteByReference uErrorCode/* 返回的错误代码 */) {
	...
	// 返回实际擦除的字节数
	public int EraseDataByEPC(String uAccessPwd/* 000000,4字节默认密码，送入字符串 */, int uBank/* 详见文件开头UHF_DATA_SECTION_XX的定义 */,
			ByteByReference uErrorCode/* 返回的错误代码 */) {
		return ns.UhfEraseDataByEPC(handle, uAccessPwd, (byte) uBank, uErrorCode, (byte) flagCrc);
	}

	// 使用前需要先选中标签
	public boolean ChangeConfig(String uAccessPwd/* 000000,4字节默认密码，送入字符串 */, int Config/* 只使用了双字节数据 */,
	...
	public boolean SetReadProtect(String uAccessPwd/* 000000,4字节默认密码，送入字符串 */,
			int Config/* 1表示启用ReadProtect，0表示取消ReadProtect */, ByteByReference uErrorCode/* 返回的错误码 */) {
	...
	public boolean ChangeEAS(String uAccessPwd/* 000000,4字节默认密码，送入字符串 */,
			int ConfigPSF/* 0x01 代表设置 PSF 位为1，0x00 代表设置 PSF 位为0 */, ByteByReference uErrorCode/* 返回的错误码 */) {
		return ns.UhfChangeEAS(handle, uAccessPwd, ConfigPSF, uErrorCode, (byte) flagCrc) != 0;
	}

	public boolean ChangeQTControl(String uAccessPwd/* 000000,4字节默认密码，送入字符串 */,
			int ReadWrite/* 0x00: Read，0x01: Write */,
			int Persistence/* 0x00: 写入标签挥发性存储区，0x01: 写入非挥发性存储区 */, String Payload/* 双字节的字符串 */,
			ByteByReference uErrorCode/* 返回的错误码 */) {
		return ns.UhfChangeQTControl(handle, uAccessPwd, ReadWrite, Persistence, Payload, uErrorCode,
				(byte) flagCrc) != 0;
	}

	// 锁定
	public boolean LockMemByEPC(String EPCXZ, String uAccessPwd, String uLockData, ByteByReference uErrorCode) {
		return ns.UhfLockMemByEPC(handle, EPCXZ, uAccessPwd, uLockData, uErrorCode, (byte) flagCrc) != 0;
	}

	// 解锁
	public boolean UnLockMemByEPC(String EPCXZ, String uAccessPwd, String uLockData, ByteByReference uErrorCode) {
